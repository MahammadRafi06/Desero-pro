#!/usr/bin/env python3
"""
GPU Performance Lab Dashboard Server - FastAPI Version

A modern web dashboard for viewing benchmark results, LLM analysis,
...
ARCHITECTURE:
    ...
    The API only manages HTTP requests/responses and routing.

    Browser <-> FastAPI (HTTP) <-> PerformanceEngine (10 domains)
                                            ↓
...
Usage:
    python -m dashboard.api.server_fastapi [--port 6970] [--data results.json]

MIGRATION FROM ORIGINAL server.py:
    This FastAPI version provides the following improvements over the original
    http.server-based implementation:
    
    1. **Modern Framework**: Uses FastAPI instead of http.server.SimpleHTTPRequestHandler
    2. **Automatic API Documentation**: 
       - Swagger UI at /docs
       - ReDoc at /redoc
    3. **Type Safety**: Pydantic models for request/response validation
    4. **Better Performance**: ASGI-based (uvicorn) with async support
    5. **CORS Support**: Built-in CORS middleware
    6. **Dependency Injection**: Clean service layer pattern
    7. **Better Error Handling**: Automatic validation and error responses
    8. **WebSocket Support**: Native support for future real-time features
...
RUNNING THE SERVER:
    # Start with default settings (port 6970)
    python -m dashboard.api.server_fastapi serve
    
    # Custom port
    python -m dashboard.api.server_fastapi serve --port 8000
    
    # Custom data file
    python -m dashboard.api.server_fastapi serve --data results.json
    
    # Custom bench root
    python -m dashboard.api.server_fastapi serve --bench-root /path/to/benchmarks
    
    # Disable auto-browser opening
    python -m dashboard.api.server_fastapi serve --no-browser
    
    # Bind to specific host (default: 0.0.0.0)
    python -m dashboard.api.server_fastapi serve --host 127.0.0.1
"""
...
from core.cost.costs import calculate_costs
from core.optimization.optimization_reports import compute_roi
from core.utils.code_diff import find_code_pair, summarize_diff
from core.profile import profile_insights
from core.optimization import optimization_stack
from core.utils import whatif as whatif_core
from core.utils import advanced_wrappers
from core.analysis.ncu_analysis import load_ncu_deepdive
from core.kernel.kernel_efficiency import score_kernels
from core.utils.warmup_audit import run_warmup_audit
from core.export.report_export import generate_html_report
...
_gpu_history_max_size = 5  # Store last 5 minutes at 1 sample/second
...
try:
    from core.llm.llm_engine import PerformanceAnalysisEngine, LLMConfig
...
class DashboardService:
    """
    Dashboard service that provides business logic for API endpoints.
    ...
    ARCHITECTURE:
        ...
        Browser <-> FastAPI (HTTP) <-> DashboardService <-> PerformanceEngine
                                                                    ↓
                                [gpu, system, profile, analyze, optimize, distributed,
                                 inference, benchmark, ai, export]
    ...
    def __init__(self, data_file: Optional[Path] = None, bench_root: Optional[Path] = None):
        ...
        self._perf_engine: Optional[PerformanceEngine] = None
        self._core_delegate: Optional[Any] = None
        self._llm_engine: Optional[Any] = None
        self._llm_advisor: Optional[Any] = None
    ...
    def get_flamegraph_comparison(self, chapter: str) -> dict:
        ...
        """
        from core.profile.profile_insights import generate_flamegraph_comparison
    ...
    def gpu_metrics_generator(self):
        ...
        while trun_off:
            ...
            snapshot = {
                ...
                #     gpu_data.get("memory_used", 0)
                #     / max(gpu_data.get("memory_total", 1), 1)
                #     * 100,
                #     1,
                # ),
                # "gpu_name": gpu_data.get("name", "Unknown"),
                "temperature": gpu_data.get("temperature") if gpu_data.get("temperature") else 0,
                "temperature_hbm": gpu_data.get("temperature_hbm") if gpu_data.get("temperature_hbm") else 0,
                "power": gpu_data.get("power") if gpu_data.get("power") else 0,
                "power_limit": gpu_data.get("power_limit") if gpu_data.get("power_limit") else 0,
                "utilization": gpu_data.get("utilization") if gpu_data.get("utilization") else 0,
                "memory_used": gpu_data.get("memory_used") if gpu_data.get("memory_used") else 0,
                "memory_total": gpu_data.get("memory_total") if gpu_data.get("memory_total") else 0,
                "memory_percent": round(
                    (gpu_data.get("memory_used") if gpu_data.get("memory_used") else 0)
                    / max((gpu_data.get("memory_total") if gpu_data.get("memory_total") else 0), 1)
                    * 100,
                    1,
                ),
                "gpu_name": gpu_data.get("gpu_name") if gpu_data.get("gpu_name") else "unknown",
            }
    ...
    def start_torch_profiler_capture(self, params: dict) -> dict:
        ...
        from core.profile.torch_profiler import TorchProfilerAutomation
    ...
    def start_hta_capture(self, params: dict) -> dict:
        ...
        from core.profile.hta_capture import HTACaptureAutomation
        from core.profile.nsight_automation import NsightAutomation
...
'''
    ...
    def get_mcp_tools(self) -> dict:
        ...
        try:
            ...
            from core.mcp.mcp_server import TOOLS
    ...
    def get_mcp_status(self) -> dict:
        ...
        try:
            ...
            from core.mcp.mcp_server import TOOLS, HANDLERS
    ...
    def call_mcp_tool(self, params: dict) -> dict:
        ...
        try:
            ...
            from core.mcp.mcp_server import HANDLERS
...
# Create FastAPI app
app = FastAPI(
    title="GPU Performance Lab Dashboard",
    description="A modern web dashboard for GPU performance analysis",
    version="2.0.0",
    lifespan=lifespan
)
...
def get_service() -> DashboardService:
    ...
    if dashboard_service is None:
        raise RuntimeError("Dashboard service not initialized")
    return dashboard_service


async def sse_generator(event_queue: queue.Queue):
    """Generate Server-Sent Events from a queue."""
    try:
        while True:
            try:
...
async def get_gpu_info():
    """Get GPU information."""
    service = get_service()
    return service.get_gpu_info()


@app.get("/api/gpu/stream")
async def stream_gpu_metrics():
    """Stream GPU metrics using Server-Sent Events (SSE)."""
    service = get_service()
    return StreamingResponse(
        service.gpu_metrics_generator(),
...
async def get_gpu_history():
    ...
    global _gpu_history
    return {"history": _gpu_history}


@app.get("/api/gpu/bandwidth")
@app.get("/api/gpu-bandwidth")
async def get_gpu_bandwidth_test():
    """Run GPU bandwidth test."""
    service = get_service()
    return service.run_gpu_bandwidth_test()


@app.get("/api/gpu/topology")
async def get_gpu_topology():
    """Get GPU topology information."""
    service = get_service()
    return service.engine.gpu.topology()


@app.get("/api/gpu/power")
async def get_gpu_power():
    """Get GPU power analysis."""
    service = get_service()
    return service.engine.analyze.power()


@app.get("/api/gpu/nvlink")
...
async def get_software_info():
    ...
    service = get_service()
    return service.get_software_info()
...
async def get_dependency_health():
    ...
    service = get_service()
    return service.get_dependency_health()


@app.get("/api/system/dependencies/check-updates")
...
@app.get("/api/system-context")
async def get_full_system_context():
    """Get full system context."""
    service = get_service()
    return service.get_full_system_context()
...
# =============================================================================
# API ENDPOINTS - Benchmark Domain
# =============================================================================
...
async def get_benchmark_data():
    """Get benchmark data."""
    service = get_service()
    return service.load_benchmark_data()
...
async def scan_all_chapters():
    """Scan all chapters and labs."""
    service = get_service()
    return service.scan_all_chapters_and_labs()


@app.get("/api/benchmark/speedtest")
async def run_speed_tests():
    """Run speed tests."""
    service = get_service()
    return service.run_speed_tests()


@app.get("/api/benchmark/network")
async def run_network_tests():
    """Run network tests."""
    service = get_service()
    return service._core.run_network_tests()


@app.get("/api/benchmark/triage")
async def get_benchmark_triage():
    """Get benchmark triage information."""
    service = get_service()
    return service.get_benchmark_triage()


# =============================================================================
# API ENDPOINTS - Profile Domain
# =============================================================================

@app.get("/api/profile/flame")
@app.get("/api/profiler/flame")
async def get_flame_graph():
    """Get flame graph data."""
    service = get_service()
    return service.get_flame_graph_data()


@app.get("/api/profile/memory")
@app.get("/api/profiler/memory")
async def get_memory_timeline():
    """Get memory timeline."""
    service = get_service()
    return service.get_memory_timeline()


@app.get("/api/profile/timeline")
@app.get("/api/profiler/timeline")
...
@app.get("/api/profiler/kernels")
async def get_kernel_breakdown():
    """Get kernel breakdown."""
    service = get_service()
    return service.get_kernel_breakdown()


@app.get("/api/profile/hta")
@app.get("/api/profiler/hta")
async def get_hta_analysis():
    """Get HTA analysis."""
    service = get_service()
    return service.get_hta_analysis()


@app.get("/api/profile/torch")
@app.get("/api/profiler/torch")
async def get_torch_profiler():
    """Get torch profiler data."""
    service = get_service()
    return service.get_torch_profiler()


@app.get("/api/profile/compile")
@app.get("/api/profiler/compile")
async def get_compile_analysis():
    """Get compile analysis."""
    service = get_service()
    return service._core.get_compile_analysis()


@app.get("/api/profile/roofline")
@app.get("/api/profiler/roofline")
async def get_roofline():
    """Get roofline data."""
    service = get_service()
    return service.get_roofline_data()


@app.get("/api/profile/list")
@app.get("/api/deep-profile/list")
async def list_deep_profile_pairs():
    """List deep profile pairs."""
    service = get_service()
    return service._core.list_deep_profile_pairs()


@app.get("/api/profile/recommendations")
@app.get("/api/deep-profile/recommendations")
...
async def load_profile_data():
    """Load profile data."""
    service = get_service()
    return service.load_profile_data()


@app.get("/api/profile/compare/{chapter}")
@app.get("/api/deep-profile/compare/{chapter}")
...
@app.get("/api/deep-profile/flamegraph/{chapter}")
async def get_flamegraph_comparison(chapter: str):
    """Get flamegraph comparison."""
    service = get_service()
    return service._core.get_flamegraph_comparison(chapter)


@app.get("/api/profiler/bottlenecks")
async def get_profiler_bottlenecks():
    """Get profiler bottlenecks."""
    service = get_service()
    return service.detect_bottlenecks()


@app.get("/api/profiler/optimization-score")
async def calculate_optimization_score():
    """Calculate optimization score."""
    service = get_service()
    return service._core.calculate_optimization_score()


# =============================================================================
# API ENDPOINTS - Analyze Domain
# =============================================================================

@app.get("/api/analyze/pareto")
@app.get("/api/analysis/pareto")
async def get_pareto_frontier():
    """Get Pareto frontier analysis."""
    service = get_service()
    return service._core.get_pareto_frontier()


@app.get("/api/analyze/tradeoffs")
@app.get("/api/analysis/tradeoffs")
async def get_tradeoff_analysis():
    """Get tradeoff analysis."""
    service = get_service()
    return service._core.get_tradeoff_analysis()


@app.get("/api/analyze/stacking")
@app.get("/api/analysis/stacking")
@app.get("/api/stacking")
async def get_optimization_stacking():
    """Get optimization stacking analysis."""
    service = get_service()
    return service._core.get_optimization_stacking()


@app.get("/api/analyze/power")
@app.get("/api/analysis/power")
async def get_power_efficiency():
    """Get power efficiency analysis."""
    service = get_service()
    return service._core.get_power_efficiency()


@app.get("/api/analyze/scaling")
@app.get("/api/analysis/scaling")
async def get_scaling_analysis():
    """Get scaling analysis."""
    service = get_service()
    return service._core.get_scaling_analysis()


@app.get("/api/analyze/leaderboards")
@app.get("/api/analysis/leaderboards")
async def get_categorized_leaderboards():
    """Get categorized leaderboards."""
    service = get_service()
    return service._core.get_categorized_leaderboards()


@app.get("/api/analysis/bottlenecks")
async def get_bottleneck_summary():
    """Get bottleneck summary."""
    service = get_service()
    return service._core.get_bottleneck_summary()


@app.get("/api/analyze/whatif")
@app.get("/api/analysis/whatif")
async def get_whatif_recommendations(
    ...
    return service._core.get_whatif_recommendations(params)


@app.get("/api/analysis/cost")
async def get_cost_analysis(
    ...
    """Get cost analysis."""
    service = get_service()
    return service._core.get_cost_analysis(gpu=gpu, custom_rate=rate)


@app.get("/api/analysis/cpu-memory")
...
async def analyze_bank_conflicts(
    stride: int = Query(1),
    element_size: int = Query(4)
):
...
async def analyze_memory_access(
    stride: int = Query(1),
    element_size: int = Query(4)
):
...
async def run_auto_tuning(
    kernel: str = Query("matmul"),
    max_configs: int = Query(50)
):
    """Run auto-tuning."""
    service = get_service()
    return service._core.run_auto_tuning(kernel, max_configs)


@app.get("/api/analysis/full-system")
...
async def analyze_energy_efficiency(
    gpu: str = Query("H100"),
    power_limit: Optional[int] = Query(None)
):
    """Analyze energy efficiency."""
    service = get_service()
    return service._core.analyze_energy_efficiency(gpu, power_limit)


@app.get("/api/analysis/multi-gpu-scaling")
async def estimate_multi_gpu_scaling(
    gpus: int = Query(8),
    nvlink: bool = Query(True),
    workload: str = Query("training")
):
    """Estimate multi-GPU scaling."""
    service = get_service()
    return service._core.estimate_multi_gpu_scaling(gpus, nvlink, workload)


@app.get("/api/analysis/optimizations")
async def get_all_optimizations():
    """Get all optimizations."""
    service = get_service()
    return service._core.get_all_optimizations()


@app.get("/api/analysis/playbooks")
async def get_optimization_playbooks():
    """Get optimization playbooks."""
    service = get_service()
    return service._core.get_optimization_playbooks()


@app.get("/api/analysis/compound")
async def calculate_compound_optimization(opts: str = Query("")):
    """Calculate compound optimization."""
    service = get_service()
    opt_list = [o.strip() for o in opts.split(",") if o.strip()]
    return service._core.calculate_compound_optimization(opt_list)


@app.get("/api/analysis/optimal-stack")
async def get_optimal_optimization_stack(
    target: float = Query(10),
    difficulty: str = Query("medium")
):
    """Get optimal optimization stack."""
    service = get_service()
    return service._core.get_optimal_optimization_stack(target, difficulty)


@app.get("/api/analysis/occupancy")
async def calculate_occupancy(
    threads: int = Query(256),
    shared: int = Query(0),
    registers: int = Query(32)
):
    """Calculate occupancy."""
    service = get_service()
    return service._core.calculate_occupancy(threads, shared, registers)


# =============================================================================
# API ENDPOINTS - Optimize Domain
# =============================================================================
...
async def list_optimization_jobs():
    """List all optimization jobs."""
    global _optimization_jobs
    return {"jobs": list(_optimization_jobs.values())}
    ...
    def run_optimization():
        try:
            ...
            for i in range(5):
                ...
                _job_events[job_id].put(event)
                job["events"].append(event)
                time.sleep(1)
            
            # Final event
            event = {"type": "complete", "message": "Optimization complete"}
            _job_events[job_id].put(event)
            _job_events[job_id].put(None)  # Sentinel
            job["status"] = "complete"
...
@app.get("/api/optimize/stream/{job_id}")
async def stream_optimization_events(job_id: str):
    """Stream optimization events using Server-Sent Events."""
    global _job_events
...
# =============================================================================
# API ENDPOINTS - AI/LLM Domain
# =============================================================================

@app.get("/api/ai/analysis")
@app.get("/api/llm-analysis")
async def load_llm_analysis():
    """Load LLM analysis."""
    service = get_service()
    return service._core.load_llm_analysis()


@app.get("/api/llm/status")
async def get_llm_status(probe: bool = Query(False)):
    """Get LLM status."""
    service = get_service()
    return service._core.get_llm_status(probe=probe)


@app.get("/api/llm/analyze-bottlenecks")
async def llm_analyze_bottlenecks():
    """LLM analyze bottlenecks."""
    service = get_service()
    return service._core.llm_analyze_bottlenecks()


@app.get("/api/llm/distributed")
async def llm_distributed_recommendations(
    nodes: int = Query(1),
    gpus: int = Query(8),
    params: float = Query(70),
    interconnect: str = Query("infiniband")
):
    """Get LLM distributed training recommendations."""
    service = get_service()
    req = {
        "num_nodes": nodes,
        "gpus_per_node": gpus,
    ...
    return service._core.llm_distributed_recommendations(req)


@app.get("/api/llm/inference")
async def llm_inference_recommendations(
    model: str = Query("llama-3.1-70b"),
    latency: float = Query(0),
    throughput: float = Query(0),
    batch: int = Query(32),
    seq: int = Query(4096)
):
    """Get LLM inference recommendations."""
    service = get_service()
    req = {
        "model": unquote(model),
        "target_latency_ms": latency or None,
        "target_throughput": throughput or None,
        "max_batch_size": batch,
        "max_sequence_length": seq
    }
    return service._core.llm_inference_recommendations(req)


@app.get("/api/llm/rlhf")
async def llm_rlhf_recommendations(
    policy: float = Query(7),
    reward: float = Query(7),
    gpus: int = Query(8)
):
    """Get LLM RLHF recommendations."""
    service = get_service()
    req = {
        "policy_size_b": policy,
        "reward_size_b": reward,
        "num_gpus": gpus
    }
    return service._core.llm_rlhf_recommendations(req)


@app.get("/api/llm/custom-query")
async def llm_custom_query(q: str = Query("")):
    """Custom LLM query."""
    service = get_service()
    return service._core.llm_custom_query(unquote(q))


@app.get("/api/llm/advisor")
async def get_llm_optimization_advice(
    model: str = Query("llama-3.1-70b"),
    goal: str = Query("throughput"),
    gpus: int = Query(8),
    training: bool = Query(True),
    ...
    return service._core.get_llm_optimization_advice(req)


# =============================================================================
# API ENDPOINTS - Export Domain
# =============================================================================

@app.get("/api/export/csv")
async def export_csv(detailed: bool = Query(False)):
    """Export benchmarks to CSV."""
    service = get_service()
    if detailed:
        csv_data = service.export_detailed_csv()
    else:
        csv_data = service.export_benchmarks_csv()
...
@app.get("/api/export/csv/detailed")
async def export_detailed_csv_endpoint():
    """Export detailed CSV."""
    service = get_service()
    csv_data = service.export_detailed_csv()
    return Response(
        content=csv_data,
        media_type="text/csv",
        headers={"Content-Disposition": "attachment; filename=benchmark_results_detailed.csv"}
    )
...
async def export_pdf():
    """Export PDF report."""
    service = get_service()
    return service._core.export_pdf_report()
...
# =============================================================================
# API ENDPOINTS - Config Domain
# =============================================================================

@app.get("/api/config/bench-root")
async def get_bench_root_info():
    """Get bench root information."""
    service = get_service()
    return service.get_bench_root_info()


@app.post("/api/config/bench-root")
async def update_bench_root(update: BenchRootUpdate):
    """Update bench root configuration."""
    service = get_service()
    params = {}
    if update.bench_root is not None:
        params["bench_root"] = update.bench_root
    if update.data_file is not None:
        params["data_file"] = update.data_file
    return service.update_bench_root_config(params)


# =============================================================================
# API ENDPOINTS - Hardware Testing
# =============================================================================

@app.get("/api/hw/disk")
async def test_disk_io(
    file_size_mb: int = Query(256),
    block_size_kb: int = Query(1024),
    tmp_dir: Optional[str] = Query(None),
    precheck_only: bool = Query(False),
...
@app.get("/api/hw/pcie")
async def test_pcie_bandwidth(
    size_mb: int = Query(256),
    iters: int = Query(10),
    precheck_only: bool = Query(False),
    dry_run: bool = Query(False),
    timeout_seconds: Optional[int] = Query(None)
...
@app.get("/api/hw/cache")
async def test_mem_hierarchy(
    size_mb: int = Query(256),
    stride: int = Query(128),
    precheck_only: bool = Query(False),
    dry_run: bool = Query(False),
    timeout_seconds: Optional[int] = Query(None)
):
...
@app.get("/api/hw/roofline")
@app.get("/api/roofline")
async def roofline_sweep(
    size_mb: int = Query(32),
    stride: Optional[List[int]] = Query(None),
    precheck_only: bool = Query(False),
    dry_run: bool = Query(False),
    timeout_seconds: Optional[int] = Query(None)
):
    ...
    if precheck_only:
        return {
            "precheck_only": True,
            "planned": {"size_mb": size_mb, "strides": stride}
        }
    if dry_run:
        return {
            "dry_run": True,
    ...
    service = get_service()
    return service._core.roofline_sweep(
        size_mb=size_mb,
        strides=stride,
        timeout_seconds=timeout_seconds
    )
...
@app.get("/api/hw/sfu")
async def test_sfu(
    elements: int = Query(64 * 1024 * 1024),
    precheck_only: bool = Query(False),
    dry_run: bool = Query(False),
    timeout_seconds: Optional[int] = Query(None)
):
    """Test special function units."""
    if precheck_only:
        return {
            "precheck_only": True,
...
async def test_network_loopback(
    size_mb: int = Query(64),
    port: int = Query(50007),
    precheck_only: bool = Query(False),
    dry_run: bool = Query(False),
    timeout_seconds: Optional[int] = Query(None)
):
...
# =============================================================================
# API ENDPOINTS - Nsight Profiling
# =============================================================================

@app.get("/api/nsight/availability")
async def get_nsight_availability():
    """Get Nsight tools availability."""
    from core.profile.nsight_automation import NsightAutomation
...
@app.get("/api/nsight/job-status")
@app.get("/api/profile/job-status")
@app.get("/api/job-status")
async def get_job_status(job_id: str = Query(...)):
    """Get job status."""
    service = get_service()
    try:
        return service._core.get_unified_job_status(job_id)
    except:
        return service._core.get_profile_job_status(job_id)


@app.get("/api/nsight/compare/nsys")
async def compare_nsys(dir: str = Query("")):
    """Compare nsys files."""
    from core import profile_insights
    result = profile_insights.compare_nsys_files(Path(dir))
    return result or {"error": "No comparable nsys files found"}


@app.get("/api/nsight/compare/ncu")
async def compare_ncu(dir: str = Query("")):
    """Compare ncu files."""
    from core import profile_insights
    result = profile_insights.compare_ncu_files(Path(dir))
    return result or {"error": "No comparable ncu files found"}


# =============================================================================
# API ENDPOINTS - Parallelism Strategy Advisor
# =============================================================================

@app.get("/api/parallelism/topology")
async def get_parallelism_topology():
    """Get parallelism topology."""
    service = get_service()
    return service._core.get_parallelism_topology()


@app.get("/api/parallelism/presets")
async def get_parallelism_presets():
    """Get parallelism presets."""
    service = get_service()
    return service._core.get_parallelism_presets()


@app.get("/api/parallelism/recommend")
async def get_parallelism_recommendations(
    model: str = Query("llama-3.1-70b"),
    batch: int = Query(1),
    seq: int = Query(2048),
    goal: str = Query("throughput"),
    ...
    return service._core.get_parallelism_recommendations(req)


@app.get("/api/parallelism/analyze-model")
async def analyze_parallelism_model(model: str = Query("")):
    """Analyze model for parallelism."""
    service = get_service()
    return service._core.analyze_parallelism_model(unquote(model))


@app.get("/api/parallelism/clusters")
async def get_cluster_presets():
    """Get cluster presets."""
    service = get_service()
    return service._core.get_cluster_presets()


@app.get("/api/parallelism/sharding")
async def get_sharding_recommendations(
    model: str = Query("llama-3.1-70b"),
    dp: int = Query(8),
    ...
    return service._core.get_sharding_recommendations(req)


@app.get("/api/parallelism/pareto")
async def get_pareto_analysis(
    model: str = Query("llama-3.1-70b"),
    cost: float = Query(4.0)
):
    """Get Pareto analysis."""
    service = get_service()
    req = {
    ...
    return service._core.get_pareto_analysis(req)


@app.get("/api/parallelism/launch")
async def get_launch_commands(
    ...
    pp: int = Query(1),
    dp: int = Query(8),
    sharding: str = Query("none"),
    script: str = Query("train.py")
):
    """Get launch commands."""
    service = get_service()
    req = {
    ...
    return service._core.get_launch_commands(req)


@app.get("/api/parallelism/calibration")
async def get_calibration_data():
    """Get calibration data."""
    service = get_service()
    return service._core.get_calibration_data()


@app.get("/api/parallelism/validate")
async def validate_parallelism_config(
    model: str = Query("llama-3.1-70b"),
    tp: int = Query(1),
    pp: int = Query(1),
    dp: int = Query(8),
    ...
    return service._core.validate_parallelism_config(req)


@app.get("/api/parallelism/optimize")
async def get_advanced_optimizations(
    model: str = Query("llama-3.1-70b"),
    ...
    return service._core.get_advanced_optimizations(req)


@app.get("/api/parallelism/profiles")
async def list_performance_profiles():
    """List performance profiles."""
    service = get_service()
    return service._core.list_performance_profiles()


@app.get("/api/parallelism/profile")
async def get_performance_profile(
    model: str = Query("llama-3.1-70b"),
    workload: str = Query("pretraining"),
    batch: int = Query(32),
    seq: int = Query(4096),
    lora: bool = Query(False),
    inference_mode: str = Query("batch")
):
    ...
    return service._core.get_performance_profile(req)
...
async def get_nccl_tuning_legacy(
    ...
    return service._core.get_nccl_tuning(req)


@app.get("/api/distributed/rlhf")
async def get_rlhf_analysis(
    model: str = Query("llama-3.1-70b"),
    algorithm: str = Query("ppo"),
    batch: int = Query(4),
    seq: int = Query(2048),
    memory: float = Query(80),
    ...
    return service._core.get_rlhf_analysis(req)


@app.get("/api/distributed/moe")
async def get_moe_config(
    model: str = Query("mixtral-8x7b"),
    experts: int = Query(8),
    gpus: int = Query(8),
    memory: float = Query(80),
    batch: int = Query(8)
    ...
    return service._core.get_moe_config(req)


@app.get("/api/distributed/long-context")
async def get_long_context_config(
    model: str = Query("llama-3.1-70b"),
    seq: int = Query(128000),
    gpus: int = Query(8),
    memory: float = Query(80),
    method: str = Query("auto")
):
    """Get long context configuration."""
    service = get_service()
    req = {
        "model": unquote(model),
        "seq_length": seq,
...
async def get_vllm_config_legacy(
    ...
    return service._core.get_vllm_config(req)


@app.get("/api/distributed/comm-overlap")
async def get_comm_overlap_analysis(
    model: str = Query("llama-3.1-70b"),
    tp: int = Query(1),
    pp: int = Query(1),
    dp: int = Query(8),
    ...
    """Get communication overlap analysis."""
    service = get_service()
    req = {
        "model": unquote(model),
        "tp": tp,
    ...
    return service._core.get_comm_overlap_analysis(req)


# =============================================================================
# API ENDPOINTS - Cluster Resilience
# =============================================================================

@app.get("/api/cluster/fault-tolerance")
async def get_fault_tolerance_config(
    params: float = Query(70),
    nodes: int = Query(1),
    gpus: int = Query(8),
    hours: int = Query(24),
    ...
    return service._core.get_fault_tolerance_config(req)


@app.get("/api/cluster/spot-config")
async def get_spot_instance_config(
    params: float = Query(70),
    cloud: str = Query("aws"),
    budget: bool = Query(True)
):
    ...
    return service._core.get_spot_instance_config(req)


@app.get("/api/cluster/elastic-scaling")
async def get_elastic_scaling_config(
    params: float = Query(70),
    nodes: int = Query(4),
    traffic: str = Query("variable")
):
    ...
    return service._core.get_elastic_scaling_config(req)


@app.get("/api/cluster/diagnose")
async def diagnose_cluster_error(error: str = Query("")):
    """Diagnose cluster error."""
    service = get_service()
    return service._core.diagnose_cluster_error({"error": unquote(error)})


# =============================================================================
# API ENDPOINTS - RLHF Specific
# =============================================================================

@app.get("/api/rlhf/methods")
async def get_rlhf_methods():
    """Get RLHF methods."""
    service = get_service()
    return service._core.get_rlhf_methods()


@app.get("/api/rlhf/config")
async def get_rlhf_config(
    method: str = Query("ppo"),
    model_size: str = Query("7"),
    gpus: str = Query("8"),
    memory_gb: str = Query("80")
):
    ...
    return service._core.get_rlhf_config(req)


@app.get("/api/rlhf/memory")
async def estimate_rlhf_memory(model_size: str = Query("7")):
    """Estimate RLHF memory requirements."""
    service = get_service()
    return service._core.estimate_rlhf_memory({"model_size": model_size})
...
@app.get("/api/roofline/interactive")
async def get_interactive_roofline():
    """Get interactive roofline data."""
    service = get_service()
    return service._core.get_interactive_roofline()


@app.get("/api/cost/calculator")
async def get_cost_calculator():
    """Get cost calculator data."""
    service = get_service()
    return service._core.get_cost_calculator()


@app.get("/api/cost/roi")
async def get_optimization_roi():
    """Get optimization ROI."""
    service = get_service()
    return service._core.get_optimization_roi()


@app.get("/api/cost/savings-header")
async def get_cost_savings_header(ops_per_day: int = Query(1000000)):
    """Get cost savings header."""
    service = get_service()
    return service._core.get_cost_savings_header(ops_per_day)


@app.get("/api/diff/{chapter}")
async def get_code_diff(chapter: str):
    """Get code diff for a chapter."""
    service = get_service()
    return service._core.get_code_diff(chapter)


@app.get("/api/efficiency/kernels")
async def get_kernel_efficiency():
    """Get kernel efficiency analysis."""
    service = get_service()
    return service._core.get_kernel_efficiency()
...
async def get_whatif_scenarios():
    ...
    service = get_service()
    return service._core.get_whatif_scenarios()


@app.get("/api/ncu/deepdive")
async def get_ncu_deepdive():
    """Get NCU deep dive analysis."""
    service = get_service()
    return service._core.get_ncu_deepdive()


@app.get("/api/intelligence/recommend")
async def get_intelligent_recommendation(request: Request):
    """Get intelligent recommendations."""
    service = get_service()
    params = dict(request.query_params)
    return service._core.get_intelligent_recommendation(params)


@app.get("/api/intelligence/distributed")
async def get_distributed_training_plan(request: Request):
    """Get distributed training plan."""
    service = get_service()
    params = dict(request.query_params)
    return service._core.get_distributed_training_plan(params)


@app.get("/api/intelligence/vllm")
async def get_intelligence_vllm_config(request: Request):
    """Get vLLM configuration from intelligence engine."""
    service = get_service()
    params = dict(request.query_params)
    return service._core.get_vllm_config(params)


@app.get("/api/intelligence/rl")
async def get_intelligence_rl_config(request: Request):
    """Get RL configuration from intelligence engine."""
    service = get_service()
    params = dict(request.query_params)
    return service._core.get_rl_config(params)


@app.get("/api/intelligence/techniques")
async def get_optimization_techniques():
    """Get optimization techniques."""
    service = get_service()
    return service._core.get_optimization_techniques()
...
async def generate_report():
    ...
    service = get_service()
    return service._core.send_html_report()


@app.get("/api/hardware-capabilities")
async def get_hw_capabilities():
    """Get hardware capabilities."""
    service = get_service()
    return service.get_hardware_capabilities()


@app.get("/api/explain/{technique}")
@app.get("/api/explain/{technique}/{chapter}")
async def get_technique_explanation(
    technique: str,
    chapter: Optional[str] = None
):
    """Get technique explanation."""
    service = get_service()
    return service._core.get_technique_explanation(unquote(technique), chapter)


@app.get("/api/explain-llm/{technique}")
@app.get("/api/explain-llm/{technique}/{chapter}")
@app.get("/api/explain-llm/{technique}/{chapter}/{benchmark}")
async def get_llm_explanation(
    ...
    service = get_service()
    return service._core.get_llm_explanation(
        unquote(technique),
        unquote(chapter) if chapter else None,
        unquote(benchmark) if benchmark else None
    )
...
async def get_batch_size_recommendations():
    """Get batch size recommendations."""
    service = get_service()
    return service.get_batch_size_recommendations()


@app.get("/api/audit/warmup")
async def run_warmup_audit_endpoint(check_recommended: bool = Query(False)):
    """Run warmup audit."""
    service = get_service()
    return service.run_warmup_audit(check_recommended)


# =============================================================================
# API ENDPOINTS - Parallelism Troubleshooting
# =============================================================================

@app.get("/api/parallelism/troubleshoot/topics")
async def get_troubleshooting_topics():
    """Get troubleshooting topics."""
    service = get_service()
    return service._core.get_troubleshooting_topics()


@app.get("/api/parallelism/troubleshoot/diagnose")
async def diagnose_training_error(error: str = Query("")):
    """Diagnose training error."""
    service = get_service()
    return service._core.diagnose_training_error({"error": unquote(error)})


@app.get("/api/parallelism/troubleshoot/nccl")
async def get_nccl_recommendations(interconnect: str = Query("nvlink")):
    """Get NCCL recommendations."""
    service = get_service()
    return service.get_nccl_recommendations({"interconnect": interconnect})


# =============================================================================
# API ENDPOINTS - POST Methods for Advanced Operations
# =============================================================================

class NsysProfileRequest(BaseModel):
    """Request model for Nsight Systems profiling."""
    target: str
    duration: Optional[int] = 5
    output_dir: Optional[str] = None


class NcuProfileRequest(BaseModel):
    """Request model for Nsight Compute profiling."""
    target: str
    metrics: Optional[List[str]] = None
    output_dir: Optional[str] = None


class TorchProfilerRequest(BaseModel):
    """Request model for PyTorch profiler."""
    target: str
    duration: Optional[int] = 5
    profile_memory: Optional[bool] = True


class KernelAnalysisRequest(BaseModel):
...
class GPUControlRequest(BaseModel):
    """Request model for GPU control operations."""
    gpu_id: Optional[int] = 0
    value: Optional[Any] = None


class BatchCalculationRequest(BaseModel):
    """Request model for batch size calculations."""
    model: str
    gpu_memory_gb: Optional[float] = 80
    precision: Optional[str] = "fp16"
    seq_length: Optional[int] = 2048


@app.post("/api/nsight/profile/nsys")
async def start_nsys_capture(request: NsysProfileRequest):
    """Start Nsight Systems capture."""
    service = get_service()
    return service._core.start_nsys_capture(request.dict())


@app.post("/api/nsight/profile/ncu")
async def start_ncu_capture(request: NcuProfileRequest):
    """Start Nsight Compute capture."""
    service = get_service()
    return service._core.start_ncu_capture(request.dict())


@app.post("/api/profiler/torch")
async def start_torch_profiler_capture(request: TorchProfilerRequest):
    """Start PyTorch profiler capture."""
    service = get_service()
    return service._core.start_torch_profiler_capture(request.dict())


@app.post("/api/profiler/hta-capture")
async def start_hta_capture(request: Dict[str, Any]):
    """Start HTA capture."""
    service = get_service()
    return service._core.start_hta_capture(request)


@app.post("/api/optimize/stop")
async def stop_optimization(request: Dict[str, str]):
    """Stop an optimization job."""
    service = get_service()
    job_id = request.get("job_id")
    return service._core.stop_optimization_job(job_id)
...
async def call_mcp_tool(request: Dict[str, Any]):
    """Call MCP tool."""
    service = get_service()
    return service._core.call_mcp_tool(request)


@app.post("/api/profiler/analyze-kernel")
async def analyze_kernel_with_llm(request: KernelAnalysisRequest):
    """Analyze kernel with LLM."""
    service = get_service()
    return service._core.analyze_kernel_with_llm(request.dict())


@app.post("/api/profiler/generate-patch")
async def generate_optimization_patch(request: Dict[str, Any]):
    """Generate optimization patch."""
    service = get_service()
    return service._core.generate_optimization_patch(request)


@app.post("/api/profiler/ask")
async def ask_profiler_ai(request: AIQueryRequest):
    """Ask profiler AI a question."""
    service = get_service()
    return service._core.ask_profiler_ai(request.dict())


@app.post("/api/parallelism/recommend")
async def post_parallelism_recommendations(request: Dict[str, Any]):
    """Get parallelism recommendations (POST version for complex queries)."""
    service = get_service()
    return service._core.get_parallelism_recommendations(request)


@app.post("/api/parallelism/sharding")
async def post_sharding_recommendations(request: Dict[str, Any]):
    """Get sharding recommendations (POST version)."""
    service = get_service()
    return service._core.get_sharding_recommendations(request)


@app.post("/api/parallelism/pareto")
async def post_pareto_analysis(request: Dict[str, Any]):
    """Get Pareto analysis (POST version)."""
    service = get_service()
    return service._core.get_pareto_analysis(request)


@app.post("/api/parallelism/launch")
async def post_launch_commands(request: Dict[str, Any]):
    """Get launch commands (POST version)."""
    service = get_service()
    return service._core.get_launch_commands(request)


@app.post("/api/ai/query")
async def run_ai_query(request: AIQueryRequest):
    """Run AI query."""
    service = get_service()
    return service._core.run_ai_query(request.query)
...
async def test_webhook(request: WebhookRequest):
    """Test webhook."""
    service = get_service()
    return service._core.test_webhook(request.dict())


@app.post("/api/webhook/send")
async def send_webhook_notification(request: WebhookRequest):
    """Send webhook notification."""
    service = get_service()
    return service._core.send_webhook_notification(request.dict())


@app.post("/api/webhooks/save")
async def save_webhooks(request: Dict[str, Any]):
    """Save webhooks configuration."""
    service = get_service()
    return service._core.save_webhooks(request)
...
async def run_benchmark(request: BenchmarkRunRequest):
    """Run a benchmark."""
    service = get_service()
    return service._core.run_benchmark(request.dict())


@app.post("/api/benchmark/verify")
@app.post("/api/verify-benchmark")
async def verify_benchmark(request: Dict[str, Any]):
    """Verify a benchmark."""
    service = get_service()
    return service._core.verify_benchmark(request)


@app.post("/api/gpu/power-limit")
async def set_gpu_power_limit(request: GPUControlRequest):
    """Set GPU power limit."""
    service = get_service()
    return service._core.set_gpu_power_limit(request.dict())


@app.post("/api/gpu/clock-pin")
async def set_gpu_clock_pin(request: GPUControlRequest):
    """Set GPU clock pin."""
    service = get_service()
    return service._core.set_gpu_clock_pin(request.dict())


@app.post("/api/gpu/persistence")
async def set_gpu_persistence(request: GPUControlRequest):
    """Set GPU persistence mode."""
    service = get_service()
    return service._core.set_gpu_persistence(request.dict())


@app.post("/api/gpu/preset")
async def apply_gpu_preset(request: Dict[str, Any]):
    """Apply GPU preset configuration."""
    service = get_service()
    return service._core.apply_gpu_preset(request)


@app.post("/api/batch/calculate")
async def calculate_batch_for_model(request: BatchCalculationRequest):
    """Calculate optimal batch size for a model."""
    service = get_service()
    return service._core.calculate_batch_for_model(request.dict())


@app.post("/api/batch/quantization")
async def get_quantization_comparison(request: Dict[str, Any]):
    """Get quantization comparison."""
    service = get_service()
    return service._core.get_quantization_comparison(request)


@app.post("/api/batch/multi-gpu")
async def get_multi_gpu_scaling_post(request: Dict[str, Any]):
    """Get multi-GPU scaling analysis."""
    service = get_service()
    return service._core.get_multi_gpu_scaling(request)


@app.post("/api/batch/cloud-cost")
async def get_cloud_cost_estimate(request: Dict[str, Any]):
    """Get cloud cost estimate."""
    service = get_service()
    return service._core.get_cloud_cost_estimate(request)


@app.post("/api/batch/deploy-config")
async def generate_deploy_config(request: Dict[str, Any]):
    """Generate deployment configuration."""
    service = get_service()
    return service._core.generate_deploy_config(request)


@app.post("/api/batch/finetune")
async def get_finetuning_estimate(request: Dict[str, Any]):
    """Get fine-tuning estimate."""
    service = get_service()
    return service._core.get_finetuning_estimate(request)


@app.post("/api/batch/llm-advisor")
async def post_llm_optimization_advice(request: Dict[str, Any]):
    """Get LLM optimization advice (POST version)."""
    service = get_service()
    return service._core.get_llm_optimization_advice(request)


@app.post("/api/batch/compound")
async def calculate_compound_optimizations(request: Dict[str, Any]):
    """Calculate compound optimizations."""
    service = get_service()

    params = await request.json()
    return service.calculate_compound_optimizations(params)


@app.post("/api/optimize/suggest")
@app.post("/api/optimize/search")
@app.post("/api/optimize/distributed")
@app.post("/api/optimize/rlhf")
@app.post("/api/optimize/vllm")
@app.post("/api/optimize/compound")
@app.post("/api/ask")
@app.post("/api/validate")
async def handle_unified_api(request: Request):
    """Handle unified API endpoints."""
    service = get_service()
    body = await request.json()
    return service._core.handle_unified_api(request.url.path, body)
...
async def root():
    ...
    return {"message": "GPU Performance Lab Dashboard API", "version": "2.0.0"}
...
def serve_dashboard(
    ...
    dashboard_service = DashboardService(data_file=data_file, bench_root=bench_root)
...
╔════════════════════════════════════════════════════════════════════════╗
║                                                                        ║
║   ⚡ GPU Performance Lab Dashboard (FastAPI)                          ║
║                                                                        ║
